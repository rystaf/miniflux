...
func (s *Storage) RemoveFeedIcon(feedID int64) error {
	var iconID int64
	if err := s.db.QueryRow(`SELECT icon_id FROM feed_icons WHERE feed_id=$1`, feedID).Scan(&iconID); err != nil {
	...
	query := `
		DELETE FROM icons
		WHERE
			id=$1 AND NOT EXISTS(SELECT * FROM feed_icons WHERE icon_id=$1)
	`
